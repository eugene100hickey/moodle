...
## Graph  
Plot a graph of lambda ($\lambda_{max}$) versus inverse temperature ($\frac{1}{T}$). Because the numbers for $\frac{1}{T}$ are so small, you should use scientific notation. Calculate the slope of this line.  
...
z %>% ggplot(aes(1/T, lambda)) + 
  ...
  labs(title = TeX("$\\lambda_{max}$ vs inverse temperature for blackbody radiation"),
       x = TeX("$\\frac{1}{T} (K^{-1})$"),
...
z %>% ggplot(aes(T^4, power)) + 
  ...
  geom_point(show.legend = F) + 
  geom_smooth(method = "lm", fullrange = T, se = F, show.legend = F) + 
  labs(title = TeX("Power per unit area versus $temperature^4$ for a blackbody"),
       x = TeX("$T^4 (K^4)$"),
       y = TeX("P(W/m^2)")) + 
